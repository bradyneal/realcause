...
def linear_gen_model_ate(request):
    ...
    def _linear_gen_model_ate(model):
        df = generate_wty_linear_scalar_data(500, alpha=2, beta=10, delta=ate)
        gen_model = DataGenModel(df, model, AutoNormal, lr=lr, n_iters=n_iters)
        ate_est = gen_model.get_ate(n_samples_per_w=100)
...
def test_linear_multi_w_outcome_model_ate(ate):
    w, t, y = generate_wty_linear_multi_w_data(500, wdim=10, delta=ate)
    gen_model = DataGenModel((w, t, y), linear_multi_w_outcome_model, AutoNormal, n_iters=1000)
