...
def get_whynot_estimator_suite_stats(n_seeds, n_samples=100, ate=2):
    ...
    for seed in range(n_seeds):
        ...
        for key, estimate in estimated_effects.items():
            ...
            print("{} < {} < {}".format(estimate.ci[0], ate, estimate.ci[1]))
    ...
    for key, estimator_data in estimates_data.items():
        ...
        print('avg_ate:', avg_ate_est)
        print('ests:', estimator_data['ate_ests'])
        print('square diffs:', np.square(estimator_data['ate_ests'] - ate))
