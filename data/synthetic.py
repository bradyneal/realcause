...
def generate_zty_linear_scalar_data(n, data_format=PANDAS, binary_treatment=False,
                                    z_2d=False, seed=0, alpha=-2, beta=2, delta=2,
    ...
    if z_2d:
        ...
        size = n
    z = np.random.normal(loc=z_mean, scale=z_std, size=size)
    t = (alpha * z) + np.random.normal(loc=t_noise_mean, scale=t_noise_std, size=size)
    if binary_treatment:
        t = t > 0
    y = (delta * t) + (beta * z) + np.random.normal(loc=y_noise_mean, scale=y_noise_std, size=size)
    return to_data_format(data_format, z, t, y)


def generate_zty_linear_multi_z_data(n, zdim=10, data_format=TORCH,
